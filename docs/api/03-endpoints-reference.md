...
container's source (the service's main module) but not called this session.

Source of truth: the service's main module at commit HEAD of Cloud Run revision `music-api-00025-2hc`.
...
| `/v1/music/jobs/{job_id}` | GET | Cloud Run identity token + `x-api-key` (creating account only; **not counted** against the daily quota) | Poll an asynchronous job created with `"async": true`; returns `status` ∈ queued/processing/complete/failed and, when complete, the full delivery `response` | live-verified 2026-09-05 (revision music-api-00057-dw4: 202 → processing → complete in 42 s, 8/8 checks, an internal operator script) |
| `/v1/music/jobs/{job_id}/run` | POST | **Cloud Tasks only** — OIDC token of the service's own runtime account, verified in-container; any other caller is refused (401/403) | Internal runner; never called by clients | live-verified 2026-09-05 (tokenless call → 403 at the edge) |

**Asynchronous mode (added 2026-09-05):** send `"async": true` in the `POST /v1/music` body. Every $0 gate
(content safety, prompt enhance, validation) still runs synchronously — a refused request is refused with the
same 400 as before — and an admitted request is answered **202** with `job_id`, `request_id`, `status: "queued"`
and a `poll` pointer. The delivery then runs on Cloud Tasks and is written to the job record; poll
`GET /v1/music/jobs/{job_id}` or receive the same envelope at `webhook_url` (correlate by `request_id`, which is
identical in the 202 and in the webhook body). Details: [11 — Webhooks and async](11-webhook-async.md).
...
| `params` | integer | Number of parameters loaded from the service's versioned parameter specification (103 on `music-api-00085-tih`; the container fails its startup control if the spec does not load) |
| `ffmpeg` | boolean | `true` iff `ffmpeg` and `ffprobe` are on PATH inside the container |
| `version` | string | Semantic build tag from `the service's main stage` FastAPI init |
...
| `languages.singing_proven` | list | Language codes whose singing state is proven in the service's versioned `language_registry` data file — read the list, it is data, not a constant; every other code in the 109-language universe is still accepted and delivered WITH its measured capability (see 14) |
...
`duration.on_miss` ∈ `accept` / `regenerate` / `trim` (default `trim`) — the values the spec declares; any other
string is refused by name. Every nested field is validated the same way (see 04, "Validation model").

**Plan-only mode — `"dry_run": true` (added 2026-09-06, live on `music-api-00085-tih`).** Add `"dry_run": true` to any
body: the request passes content safety, prompt enhance, validation, routing and prompt compilation exactly like a
paid request, then returns the compiled plan and stops — no Lyria call, no render, no storage, no webhook, no job,
nothing billable. The 200 carries `prompt_sent`, `generation_payload`, `bindings`, `route`, `language_capability`,
`render_settings` (what the render stage WOULD do: conform / master / post-edits / export / stems) and
`request_parameters_received`; it never carries `tracks`. It takes precedence over `async`. Use it to preview a form,
to validate a request before committing a paid generation, and in integration tests. It counts as one request
against the key's daily quota. Envelope reference: [06 — Response envelope](06-response-envelope.md#the-dry-run-envelope--dry_run-true-2026-09-06).

```json
{"prompt": "late-night small-combo jazz, brushed drums, upright bass", "genres": ["jazz"], "dry_run": true}
```
...
```json
{
    ...
    "tracks": [
        {
            "public_url": "https://storage.googleapis.com/playmusicprompts-music-studio/deliveries/takes/2026/09/01/lyria_20260901_084334_7decbad8_t0.mp3",
            "gcs_uri":    "gs://playmusicprompts-music-studio/deliveries/takes/2026/09/01/lyria_20260901_084334_7decbad8_t0.mp3",
            "take": 1
        }
    ],
    ...
    "auth": {
        "account_id": "<the key's account_id, as minted with the operator's key-administration tool>",
        "key_id": "<the key's key_id>",
        "limit": {"rate_per_min": 60, "daily_quota": 100, "used_today": 1}
    }
}
```

(`auth.limit` on a counted success is `{rate_per_min, daily_quota, used_today}` — `api_keys.check_and_count`; the
numbers are the key's own limits, whose defaults are `DEFAULT_RATE_PER_MIN` 60 / `DEFAULT_DAILY_QUOTA` 100 in
`the service's api_keys stage`; until 2026-09-06 a legacy bootstrap key could appear here as
`{"note": "legacy bootstrap key, unlimited"}` — that path was retired, see 02-authentication.)
...
Polls the verdict of the optional originality gate that was requested on a delivery
...
```json
{
    ...
    "verdict": {
        ...
        "axis1_monotony": {
            ...
            "lag_scan": {"max_block_repeat_fraction": 0.031, "bar": 0.14419, "flagged": false},
            "verdict": "PASS",
...
`bars_status` summarises the two axes (`CALIBRATED` / `PARTIALLY_CALIBRATED` / `NOT_CALIBRATED`). **Both
axes are CALIBRATED since 2026-09-10**, so a single-take delivery — where only axis 1 can run — now
returns a real `PASS` or `MONOTONE_FLAGGED` rather than `NOT_CALIBRATED`. Axis 1 gates on the **lag-scan**
statistic against a bar chosen by the material's own genre (`corpus_genre`); applying one genre's bar to
another genre is a scope violation the artefact refuses by name, so a request whose genre maps to no
calibrated bucket still reports `NOT_CALIBRATED` with `refused_no_bucket_for_genre`. Read
`bars_status_per_axis` and each axis's own `verdict`; never gate on the summary alone. Full field table:
