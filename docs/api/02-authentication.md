...
- Displayed **once**, at mint time. The server persists only its SHA-256 hash under
  `gs://playmusicprompts-apikeys/apikeys/<hash>.json` — a bucket the API can READ and CANNOT WRITE
  (moved out of the service bucket on 2026-09-10 so that no runtime identity can forge a key record;
  source: `the service's api_keys stage` `KEY_BUCKET`, resolved from `the service's internal data/storage_layout.json`).
...
| `limits.rate_per_min` | Requests per rolling 60-second window per key (default **60**, and the per-key value may not be set above **100** — the operator's ceiling) |
| `limits.daily_quota` | Requests per UTC calendar day per key (default **100**, and the per-key value may not be set above **100** — the operator's ceiling). **Corrected 2026-09-11:** this row previously said 1000, which was ten times the real default; the service's own default is 100 and the ceiling is 100, so a plan built on 1000 requests a day would have started failing with 429 at the hundredth |
| `disabled` | If true, every request 401s until re-enabled |
| `created_at` | ISO-8601 UTC mint timestamp |

### Legacy key exception — RETIRED 2026-09-06

Until 2026-09-06 an `API_KEY` environment variable on the Cloud Run revision was honoured as a legacy
bootstrap key mapped to a synthetic `legacy` account with unlimited allowance. That path no longer exists:
every caller authenticates through the key store, every key carries its per-account limits, and a leftover
`API_KEY` on the service makes the container REFUSE TO START (fail-closed) so the plaintext secret cannot linger
in the environment. Operators removing it: `gcloud run services update music-api --region=us-central1
--remove-env-vars API_KEY`. Source: the service's main path and the boot guard above it.

## Minting a key (operator only)

There is a single admin CLI at the operator's key-administration tool. It must run under a Google
identity that has read/write on `gs://playmusicprompts-apikeys/apikeys/` (the operator-only key bucket; the runtime identity holds read-only).
...
in the service's api_keys path and the service's main path:
...
```
gs://playmusicprompts-music-studio/
