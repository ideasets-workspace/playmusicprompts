# PlayMusicPrompts Music API — Integration Documentation
...
You send a text prompt (and optionally a full custom lyric plus up to 101 parameters). The API
generates music with Google Vertex AI Lyria, conforms the delivered audio to the duration you asked
for, masters it to a broadcast-ready loudness target, and — when you requested sung custom lyrics —
...
**Instrumental** — a music bed with no vocals, delivered in about 25 seconds to 2.5 minutes.

**Sung with your exact lyrics** — a full song where a voice sings the words you supplied verbatim,
...
| 04 | [04-request-body-full.md](04-request-body-full.md) | While building your UI. All 101 parameters with their control type (text/slider/combo/chips/toggle), allowed values, and validation rules. This is your form-generation reference. |
...
**Layer 1 — a Google identity token.** This proves your caller has permission to reach the service
...
**Layer 2 — an API key.** This identifies your tenant and enforces your rate limit and daily
...
a list of every field the endpoint does accept — so a typo produces an immediately actionable
...
Both are **V4 signed URLs** — any client can GET them with no credentials, so they go straight into
...
**`caller_fixable`** — your request is wrong. The `refused` array names every violated field. Fix
the request and retry. Never retry unchanged.

**`upstream`** — the music generator refused or failed. **Do not retry the same prompt** — a model
...
**`ours`** — the service itself broke. Retry up to three times with exponential backoff (2 s, 8 s,
...
Every response — success or failure — carries a `request_id`. **Log it.** When you report a
...
If any is missing, ask before you start — the documents cross-reference each other and a missing
...
surface. If the parameter count that endpoint reports differs from the 101 documented here, ask the
