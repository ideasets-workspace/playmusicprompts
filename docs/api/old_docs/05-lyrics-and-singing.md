...
The separated stem is **analysis-only** (the platform's stem-shipping policy). It is stored under `gs://playmusicprompts-content/stems/`
...
- **SER (Syllable Error Rate)** — only when a sourced syllabifier exists for the language. Today
  `tr` has one; `en` reports `NOT_RUN` with the reason.
...
"lyrics_verification": {
    ...
    "protocol": {
        ...
        "stem_path": "gs://playmusicprompts-content/stems/<request>_vocals.wav",
