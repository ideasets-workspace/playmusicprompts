...
export function createAuroraAsset(THREE) {
  ...
  fiberGeometry.boundingBox.expandByScalar(.8);
  ...
  const common = /* glsl */`
    ...
    vec3 breathe(vec3 p) {
      ...
      p.x *= 1. + freedom * uDrive * .09;
      p.z *= 1. + freedom * uDrive * .14;
      p.x += sin(p.y * 1.17 + p.z * .57 + uFlow) * freedom * (.030 + uDrive * .15);
      p.z += sin(p.x * .8 + p.y * .63 + uFlow * .79) * freedom * (.040 + uDrive * .22);
      p.y += freedom * uDrive * .10;
      p.y += sin(p.z * .86 + uFlow * .71) * freedom * (.018 + uDrive * .065);
      return p;
    }
    float electricalFront(float progress) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        float front = age * .62;
        float delta = (progress - front) * 34.;
        light += exp(-delta * delta) * uPowers[i] * step(0., age) * (1. - smoothstep(1.3, 1.9, age));
      }
      return min(light, 1.8);
    }
  `;
  ...
  function createPoints(name, points, meta, type) {
    ...
    geometry.boundingBox.expandByScalar(type === 'cells' ? .8 : .6);
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main() {
          ...
          #else
            gl_FragColor = vec4(colour, halo * vLight * vDepth * (.7 + uIntensity * .5));
          #endif
        }
      `,
      transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, toneMapped: false
  ...
  function update(frame = {}) {
    ...
    dust.geometry.setDrawRange(0, frame.quality === 'low' ? 480 : dustCount);
    if (frame.motion === false) return;
    const dt = clamp(frame.dt ?? 1 / 60, 0, .05);
    elapsed += dt;
    ...
    if (frame.onset === true && frame.active === true) {
    ...
    group.rotation.y = Math.sin(flow * .20) * (.10 + drive * .10);
    group.rotation.x = Math.sin(flow * .27) * drive * .025;
  }
