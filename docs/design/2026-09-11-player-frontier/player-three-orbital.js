...
export function createOrbitalAsset(THREE) {
  ...
  const horizonMaterial = material(new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      vec4 accretion(vec3 p, float distanceTravelled) {
        vec3 normal = normalize(vec3(.045, .947, .32));
        ...
        float shear = uTime * (1.0 + uAudio.w * .26) / pow(max(r, 1.), 1.38);
        vec2 flow = vec2(a * 3.25 - shear * 4.0 + r * 1.7, r * 6.0);
        float n = turbulence(flow + vec2(0., uAudio.x * .19));
        float thread = pow(.5 + .5 * sin(r * 112. + n * 17. + a * 3.0 - shear * 2.0), 3.0);
        float veins = pow(noise(vec2(a * 7. - shear * 5., r * 27. + n * 4.)), 2.0);
        ...
        illumination *= .64 + uIntensity * .42 + uAudio.y * .24 + uPulse * .19;
      ...
      float front(float age, float strength, vec2 p) {
        ...
        return ribbon * exp(-age * 1.28) * strength * breaks;
      }
      void main() {
        vec3 origin = vec3(0., 0., 9.);
        vec3 direction = normalize(vec3(vPlane, 0.) - origin);
        vec3 point = origin;
        vec3 diskNormal = normalize(vec3(.045, .947, .32));
        ...
        for (int i = 0; i < 56; i++) {
          ...
          vec3 acceleration = -point * (1.12 / max(radius * radius * radius, .32));
  ...
  function update(frame = {}) {
    ...
    starGeometry.setDrawRange(0, lowQuality ? 576 : starCount);
    ...
    const dt = Math.max(0, Math.min(.05, Number(frame.dt) || 0));
    const bass = clamp(frame.bass), mid = clamp(frame.mid), treble = clamp(frame.treble), energy = clamp(frame.energy);
    uniforms.uTime.value += dt * (frame.active ? .46 + energy * .55 : .105);
