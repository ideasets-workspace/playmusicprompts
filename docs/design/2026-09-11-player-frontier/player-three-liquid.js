...
export function createLiquidAsset(THREE) {
  ...
  const shaderCommon = `
    ...
    float corridorTwist(float d) { return sin(d * .032 + uPhase * .032) * .26 + d * .009; }
    float corridorRadius(float d) { return 4.75 + .17 * sin(d * .08 - uPhase * .13) + uBass * .13; }
  ...
  `, `
    ...
    void main() {
      ...
      float softbox = exp(-pow((reflected.y+.21)/.145,2.0)) * (.7+.3*abs(reflected.z));
      float whiteStrip = exp(-pow((reflected.x-.46)/.076,2.0));
      float rimStrip = exp(-pow((reflected.x+.63)/.11,2.0));
      float grazing = pow(abs(dot(reflected,normalize(vec3(-.4,.78,.48)))),48.0);
  ...
  function makeRibs(accent) {
    ...
    `,`
      ...
      void main() {
        ...
        if(dot(n,eye)<0.0)n=-n;
        vec3 reflected=reflect(-eye,n);
        float spec=exp(-pow((reflected.y+.18)/.11,2.0))+.36*exp(-pow((reflected.x-.42)/.065,2.0));
        float glint=pow(abs(dot(reflected,normalize(vec3(.4,.7,.5)))),60.0);
        ${accent ? `
  ...
  const streaks=mesh('peripheral-flight-filaments',streakGeometry,shader(`
    ...
    void main(){
      ...
      vDepth=d;vEnd=aEnd;
      vec3 p=corridorPoint(d+aEnd*(.28+uEnergy*.75),aAngle,aRadius);
      gl_Position=projectionMatrix*modelViewMatrix*vec4(p,1.0);
    }
  `,`
    ...
    void main(){
      ...
      gl_FragColor=vec4(color,fade*(1.0-vEnd*.9));
    }
  ...
  function update(frame={}) {
    ...
    if(quality!==diagnostics.quality){streakGeometry.setDrawRange(0,quality==='low'?180:streakCount*2);diagnostics.quality=quality;}
    if(frame.motion===false)return;
    const delta=bounded(frame.dt,.075), ease=1-Math.exp(-delta*6), sounding=!!frame.active;
    ...
    elapsed+=delta;
    travel=(travel+delta*(.9+uniforms.uEnergy.value*4.4+uniforms.uBass.value*.6))%length;
