...
function createController() {
  ...
  const directorCalls = {manual: 0, compare: []};
  const node = key => {
  ...
  const context = {store: bridge, VISUAL_DEFAULTS, VISUAL_PRESETS, VISUAL_STORAGE_KEY,
    ...
    onManualEdit: () => {directorCalls.manual++;}, onCompareChange: value => directorCalls.compare.push(value),
  ...
  return {controller, store, messages, adapter, directorCalls,
...
check('Prism framing covers both endpoints while retaining its safe camera position and exact default', () => {
  ...
  const context = {THREE, camera, controls, collection: {view}, host: {clientWidth: 460, clientHeight: 200},
    model: 'liquid', look: {...VISUAL_DEFAULTS}, undoCameraPerformance() {}};
...
check('Actual Studio edits notify manual bypass and close temporary Compare before changing saved settings', () => {
  const {controller: ui, directorCalls: calls, store} = createController();
  assert.equal(calls.manual, 0); assert.deepEqual(calls.compare, []);
  ui.patch('exposure', 1.4); assert.equal(calls.manual, 1);
  ui.compare(); assert.deepEqual(calls.compare, [true]);
  ui.patch('bloom', 1.2); assert.deepEqual(calls.compare, [true, false]); assert.equal(calls.manual, 2);
  assert.equal(ui.inspect().preview.exposure, 1.4); assert.equal(ui.inspect().preview.bloom, 1.2);
  ui.compare(); ui.select('dreamstate'); assert.deepEqual(calls.compare, [true, false, true, false]);
  assert.equal(calls.manual, 3); assert.deepEqual(store.read().settings, VISUAL_PRESETS.find(item => item.id === 'dreamstate').settings);
});
