...
check('Three injected shader helpers cannot collide with av-prefixed custom helpers', () => {
  const bundle = fs.readFileSync(new URL('../../../website_html_templates_lab/vendor/three/build/three.module.js', import.meta.url), 'utf8');
...
check('Owned geometry and material dispose once and disposed assets cannot resume updates', () => {
  ...
  assert.equal(geometryDisposals, 1); assert.equal(materialDisposals, 1); assert.equal(atmosphere.group.children.length, 0);
});
