...
import * as REAL_THREE from '../../../website_html_templates_lab/vendor/three/build/three.module.js';
import {createAudioFeatures} from '../../../website_html_templates_lab/player-three-features.js';
import {getWorld} from '../../../website_html_templates_lab/player-three-worlds.js';
import {VISUAL_DEFAULTS,VISUAL_SCHEMA,sanitizeVisualSettings} from '../../../website_html_templates_lab/player-three-visual-settings.js';
import {AETHER_DEFAULTS,sanitizeAether} from '../../../website_html_templates_lab/lab-aether-settings.js';

const website=new URL('../../../website_html_templates_lab/',import.meta.url);
...
const names=['THREE','AETHER_DEFAULTS','sanitizeAether','createSceneCollection','createAudioFeatures','createOpticsPass','getWorld','OrbitControls','EffectComposer','RenderPass','UnrealBloomPass','OutputPass','ShaderPass','VISUAL_DEFAULTS','sanitizeVisualSettings','Reflector','RoomEnvironment'];
...
const prefix=names.map(name=>['THREE','VISUAL_DEFAULTS','AETHER_DEFAULTS'].includes(name)
  ...
  : ['createSceneCollection','createAudioFeatures','createOpticsPass','getWorld','sanitizeVisualSettings','sanitizeAether'].includes(name)
...
function canvas(){
  const owner=current;
  return {width:1,height:1,style:{},listeners:new Map(),ownerDocument:globalThis.document,getRootNode(){return this.ownerDocument;},setAttribute(){},addEventListener(type,fn){this.listeners.set(type,fn);},removeEventListener(type){this.listeners.delete(type);},
...
const deps={THREE:{...REAL_THREE,WebGLRenderer:RendererBoundary,PMREMGenerator:PMREMBoundary,TextureLoader:TextureBoundary},
  createSceneCollection(scene,options){current.collectionOptions=options;const real=realCollection(scene,options);current.collection=real;return {
    ...
    update(frame){current.frames.push({...frame,spectrum:Array.from(frame.spectrum),look:{...frame.look},labLook:{...frame.labLook}});real.update(frame);},
