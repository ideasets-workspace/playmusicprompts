...
const lab=resolve(root,'website_html_templates_lab');
...
check('defaults are complete and independent',()=>{
 assert.equal(AETHER_KEY,'pmp.lab.aether.v1');assert.equal(Object.keys(AETHER_DEFAULTS).length,8);
...
check('Director persists complete Aether cue appearance under lab namespace',()=>{
 const storage=adapter(),store=createDirectorStore(storage);assert.equal(DIRECTOR_STORAGE_KEY,'pmp.lab.room.director.v1');const result=store.patch(scored);assert.equal(result.ok,true);assert.notEqual(result.accepted,false);assert.deepEqual(store.read().cues.map(c=>c.labLook),scored.cues.map(c=>c.labLook));
});
...
check('new copied runtime stores have no original namespace literals',()=>{
 for(const name of readdirSync(lab).filter(name=>name.endsWith('.js'))){const source=readFileSync(resolve(lab,name),'utf8');assert.equal(/['"`]pmp\.(?!lab\.)/.test(source),false,name);assert.equal(/['"]pmp-local-audio['"]/.test(source),false,name);}
});
check('independent port and launch routes remain configured',()=>{
 for(const name of ['START-PLAYER.cmd','START.cmd','server.mjs','player-launch.html']){const source=readFileSync(resolve(lab,name),'utf8');assert.ok(source.includes('4175'),name);assert.equal(source.includes('4173'),false,name);}
 for(const name of ['START-PLAYER.cmd','START.cmd'])assert.match(readFileSync(resolve(lab,name),'utf8'),/set "PORT=4175"/);
});
check('lab entry identity and local import map remain independent',()=>{
 const source=readFileSync(resolve(lab,'player-three.html'),'utf8');assert.match(source,/data-pmp-edition="aether-lab"/);assert.match(source,/"three":"\.\/vendor\/three\/build\/three.module.js"/);assert.equal(/\.\.\/website_html_templates\//.test(source),false);
});
