...
check('Direct touch is finite, independent of music, and keeps a released pressure tail',()=>{
  const a=createAetherAsset(THREE),u=a.group.children[0].material.uniforms;
  a.update({...frame,active:false,onset:false,touch:{x:.6,y:-.4,strength:.9,active:true,serial:4}});
  assert.equal(u.aTouchStrength.value,.9);assert.deepEqual(a.diagnostics().touch.point,[4.2,-1.6,-9]);
  assert.equal(a.diagnostics().events,0);assert.equal(a.diagnostics().signals.energy,0);
  a.update({...frame,active:false,touch:{x:.6,y:-.4,strength:.45,active:false,serial:4}});
  assert.equal(u.aTouchStrength.value,.45);assert.equal(a.diagnostics().touch.active,false);
  a.update({...frame,touch:{x:Infinity,y:-5,strength:9,active:true,serial:NaN}});
  assert.equal(a.diagnostics().touch.x,0);assert.equal(a.diagnostics().touch.y,-1);assert.equal(u.aTouchStrength.value,1);assert.equal(a.diagnostics().touch.serial,0);
  a.update(frame);assert.equal(u.aTouchStrength.value,0);assert.deepEqual(a.diagnostics().touch.point,[0,0,-9]);a.dispose();
});
check('Motion off retains the touched shape and clocks until resume releases it',()=>{
  const a=createAetherAsset(THREE);
  a.update({...frame,touch:{x:.5,y:.2,strength:.8,active:true,serial:2}});
  const before=a.diagnostics();
  for(let i=0;i<10;i++)a.update({...frame,motion:false,touch:{x:-1,y:-1,strength:0,serial:9}});
  const after=a.diagnostics();
  assert.deepEqual({...after.touch,frozen:false},before.touch);assert.equal(after.phase,before.phase);assert.equal(after.clock,before.clock);
  a.update(frame);assert.equal(a.diagnostics().touch.strength,0);a.dispose();
});
check('Touch force uses the actual camera ray in the transformed environment',()=>{
  const a=createAetherAsset(THREE),scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(38,1.7,.1,100);
  scene.add(a.group);a.group.position.set(2,2.75,-1);a.group.rotation.set(.1,.35,-.03);a.group.scale.set(1.1,.9,1.2);
  camera.position.set(4,5,8);camera.lookAt(0,2.75,-5);camera.updateMatrixWorld(true);scene.updateMatrixWorld(true);
  a.update({...frame,touch:{x:.5,y:-.2,strength:1,active:true,serial:1}});
  a.group.children[0].onBeforeRender({},scene,camera);
  const ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2(.5,-.2),camera);
  const world=new THREE.Vector3().fromArray(a.diagnostics().touch.point).applyMatrix4(a.group.matrixWorld);
  assert.ok(world.distanceTo(ray.ray.at(12,new THREE.Vector3()))<1e-10);
  const held=a.diagnostics().touch.point;a.update({...frame,motion:false});camera.position.x+=3;camera.updateMatrixWorld(true);a.group.children[0].onBeforeRender({},scene,camera);
  assert.deepEqual(a.diagnostics().touch.point,held);a.dispose();
});
...
check('GPU and analytical paths receive the same touch target without generating audio events',()=>{
  const r=rendererDouble(),g=createAetherAsset(THREE,{renderer:r.renderer}),a=createAetherAsset(THREE);
  const input={...frame,active:false,touch:{x:-.35,y:.4,strength:.72,active:true,serial:11}};
  g.update(input);a.update(input);
  assert.deepEqual(g.diagnostics().touch,a.diagnostics().touch);
  assert.equal(g.diagnostics().events,0);assert.equal(a.diagnostics().events,0);
  assert.equal(g.group.children[0].material.uniforms.aTouchStrength.value,.72);
  assert.match(g.group.children[0].material.vertexShader,/p \+= aeTouchOffset\(p\)/);
  g.dispose();a.dispose();r.initialTarget.dispose();
});
