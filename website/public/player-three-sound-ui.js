...
export function createSoundControls({audio,storage,toast,onChange}){
  ...
  let settings=sanitizeSound(storage.get('pmp.website.room.sound',{})),rack=null,context=null,source=null;
  let saved=storage.get('pmp.website.room.soundPresets',[]);if(!Array.isArray(saved))saved=[];saved=saved.filter(p=>p&&typeof p.name==='string'&&typeof p.id==='string').slice(0,32).map(p=>({...p,name:p.name.slice(0,48),settings:sanitizeSound(p.settings)}));
  ...
  function apply(){settings=sanitizeSound(settings);rack?.apply(settings);synchronize();storage.set('pmp.website.room.sound',settings);onChange?.();}
