...
export function createDirectorControls({container, getConfig, commit, getSnapshot, getTrack, audio, getLive, captureFrame, toast, seek}) {
  ...
  on(q('#director-capture-form'), 'submit', event => {
    ...
    const snapshot = getSnapshot(), cue = {id: existing?.id || cueId(), time, duration, label: q('#director-cue-label').value.trim().slice(0, 48) || `Moment ${current.cues.length + (existing ? 0 : 1)}`, scene: getScene(snapshot.scene).id, settings: sanitizeVisualSettings(snapshot.settings), ...(['aether','tidal','monolith'].includes(snapshot.scene)&&snapshot.labLook?{labLook:{...snapshot.labLook}}:{})};
