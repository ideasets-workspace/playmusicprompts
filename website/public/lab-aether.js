...
export function createAetherAsset(THREE, { renderer } = {}) {
  ...
  const touchRay = new THREE.Raycaster(), touchCursor = new THREE.Vector2(), touchInverse = new THREE.Matrix4();
  ...
  const uniforms = {
    ...
    aTouchPoint: { value: new THREE.Vector3(0,0,-9) }, aTouchAxis: { value: new THREE.Vector3(0,0,-1) }, aTouchStrength: { value: 0 },
  ...
  const shapeShader = /* glsl */`
    ...
    uniform vec3 aTouchPoint,aTouchAxis;
    uniform float aTouchStrength;
    ...
    vec3 aeTouchOffset(vec3 p) {
      vec3 delta=p-aTouchPoint;
      float distance=length(delta);
      float envelope=exp(-distance*distance*.035)*aTouchStrength;
      return (delta/max(distance,.6)*1.5+cross(aTouchAxis,delta)*.34)*envelope;
    }
    ...
    vec3 aeShape(vec2 uv) {
      ...
      // Five narrow channels part and reunite inside each bank. The varying
      // branch widths and depth knots replace a single uniform ribbon fan.
      float branch=floor(fract(lane*3.)*5.)-2.;
      float fibre=fract(lane*15.)-.5;
      float fork=.42+.58*pow(.5+.5*sin(t*12.+family*1.7-aPhase*.09),2.);
      float forkAngle=.4+family*.6+sin(t*7.+aPhase*.1)*.38;
      vec2 axis=vec2(cos(forkAngle),sin(forkAngle));
      silk.xy+=axis*(branch*bankWidth*.38*fork);
      silk.xy+=vec2(-axis.y,axis.x)*fibre*bankWidth*(.13+.10*sin(t*17.+branch));
      silk.xy+=axis*sin(t*35.+branch*1.9+aPhase*.22)*(.025+t*.13)*(1.+aTurbulence*.55);
      silk.xy += vec2(sin(t*61.+seed*8.+aPhase*.24),cos(t*47.+seed*9.-aPhase*.19)) * (.02+t*.09)*aTurbulence;
      silk.z += ribbon*.5;
      ...
      // Corrugated nested walls leave hollows between illuminated ridges;
      // their width changes independently from the broad cavity silhouette.
      radius += (seed-.5)*(.3+t*2.4);
      radius += sin(angle*5.+t*17.-aPhase*.16)*(.12+t*.64)*(1.+aTurbulence*.25);
      ...
      float braid=floor(fract(lane*2.)*4.)-1.5;
      float braidFibre=fract(lane*8.)-.5;
      float knot=t*24.+braid*1.55+aPhase*.21;
      helix.xy+=vec2(cos(knot),sin(knot)*.67)*(braid*.15+braidFibre*.12)*(.3+t*2.1);
      helix.xy += vec2(sin(t*44.+thread*4.),cos(t*39.+thread*6.))*(.04+t*.13)*aTurbulence;
  ...
  const renderVertex = /* glsl */`
    ...
    #endif
    varying float aeColour, aeLight, aeFocus, aeSeed, aeCoverage, aeDepthFade;
    void main() {
      vec2 lookup = aLookup;
      #if AE_KIND == 3
      ...
      float vein=pow(max(.5+.5*sin(lookup.x*57.+sin(lookup.x*17.+row*.07)-aPhase*.57),0.),14.);
      #if AE_KIND == 2
      ...
      // The far trajectories dissolve into the medium instead of accumulating
      // into three glaring, apparently disconnected jet endpoints.
      aeDepthFade=1.-smoothstep(.74,1.,lookup.x);
      aeLight = .10 + sweep*.24 + vein*.25 + aTreble*(.035+aeSeed*.11) + aSpectrum[int(mod(row,64.))]*.13 + pulse*1.4;
      #if AE_KIND == 2
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main() {
          ...
          #endif
          gl_FragColor=vec4(colour*(.80+aIntensity*.42)*aGlow,clamp(alpha*aeDepthFade,0.,.92));
        }
      `,
  ...
  function locateTouch(camera=touchCamera) {
    ...
    touchInverse.copy(group.matrixWorld).invert();
    touchCursor.set(touch.x,touch.y);
    touchRay.setFromCamera(touchCursor,camera);
    touchRay.ray.at(12,uniforms.aTouchPoint.value).applyMatrix4(touchInverse);
    uniforms.aTouchPoint.value.clampScalar(-55,55);
    uniforms.aTouchAxis.value.copy(touchRay.ray.direction).transformDirection(touchInverse);
  }
  filaments.onBeforeRender=(_renderer,_scene,camera)=>{if(!disposed&&lastMotion)locateTouch(camera);};
  let lastMotion=true;
  ...
  function update(frame={}) {
    ...
    lastMotion=motion;
    if(motion){
      touch.x=value(frame.touch,'x',0,-1,1);touch.y=value(frame.touch,'y',0,-1,1);
      touch.strength=value(frame.touch,'strength',0,0,1);touch.active=frame.touch?.active===true;
      touch.serial=Math.floor(value(frame.touch,'serial',0,0,Number.MAX_SAFE_INTEGER));
      uniforms.aTouchStrength.value=touch.strength;
      if(touchCamera)locateTouch();
      else uniforms.aTouchPoint.value.set(touch.x*7,touch.y*4,-9);
    }
  ...
  function diagnostics() {
    return {name:group.name,mode,fallbackReason:reason,count,strands:count/SIDE,lineSegments:filaments.geometry.drawRange.count/2,
      ...
      touch:{...touch,point:uniforms.aTouchPoint.value.toArray(),axis:uniforms.aTouchAxis.value.toArray(),frozen:!lastMotion},
  ...
  function dispose() {
    ...
    filaments.onBeforeRender=()=>{};touchCamera=null;
