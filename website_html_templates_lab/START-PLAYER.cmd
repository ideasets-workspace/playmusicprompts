...
set "PORT=4175"
cd /d "%~dp0"
powershell -NoProfile -Command "try { $pmpResponse = Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:4175/player-three.html' -TimeoutSec 2; if ($pmpResponse.StatusCode -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }"
if not errorlevel 1 (
  start "" "http://127.0.0.1:4175/player-three.html"
  exit /b 0
)
set "PMP_NODE="
...
echo Open http://127.0.0.1:4175/player-three.html in your browser.
