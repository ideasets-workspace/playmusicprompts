...
export function createPerformanceDirector({container,storage,getRoom,getBase,getScene,getManualScene,getLabBase=()=>undefined,getTrack,audio,seek,onScene,toast,onActivate=()=>{}}){
  ...
  const controls=createDirectorControls({container,getConfig:()=>config,commit,getSnapshot:()=>({settings:{...(comparing?VISUAL_DEFAULTS:live?.settings||getBase())},scene:getScene(),...(['aether','tidal','monolith'].includes(getScene())?{labLook:{...(live?.labLook||getLabBase())}}:{})}),getTrack,audio,seek,getLive:()=>live,captureFrame,toast});
  ...
  function tick(now=performance.now()){
    ...
    const labLook=live.labLook||getLabBase();if(labLook)room.setLabSettings(labLook);
