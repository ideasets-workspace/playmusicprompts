...
import {createWorldTouch} from './world-touch.js';
...
export function createListeningRoom(host,onError){
  ...
  function choreographCamera(dt,time){
    ...
    const cameraLook=['aether','tidal','monolith'].includes(model)&&labLook.journey?{mode:'journey',amount:.85}:performanceLook.camera;const {mode}=cameraLook;const amount=cameraLook.amount*cameraHandback;if(mode==='still'||amount===0)return;
    if(mode==='journey'){const travel=model==='monolith'?[.8,.10,.45]:model==='tidal'?[1.2,.12,.7]:[1.8,.5,1.4];cameraOffset.set(Math.sin(cameraPhase*.085)*amount*travel[0],Math.sin(cameraPhase*.13)*amount*travel[1],Math.sin(cameraPhase*.11)*amount*travel[2]);camera.position.add(cameraOffset);camera.lookAt(controls.target);}
    else if(mode==='breathe'||collection.view?.orbit===false){cameraZoomFactor=1+Math.sin(cameraPhase*.29)*amount*.07;camera.zoom*=cameraZoomFactor;camera.updateProjectionMatrix();if(mode==='arc'){cameraRoll=Math.sin(cameraPhase*.19)*amount*.035;camera.rotation.z+=cameraRoll;}}
  ...
  function tick(time){if(disposed)return;requestAnimationFrame(tick);if(lost||document.hidden)return;const limit=quality==='low'||autoLow?32:16;if(time-lastRender<limit)return;const dt=Math.min(.05,(time-last)/1000);last=time;lastRender=time;
    ...
    collection.update({time:phase,dt:visualDt,bass,mid,treble,energy:recordEnergy,motion,intensity,active,spinVelocity,...musical,spectrum:visualSpectrum,pulse:musical.pulse*look.pulseGain,onset:look.pulseGain>0&&musical.onset,look,labLook,touch:worldTouch.sample(dt),quality:autoLow?'low':quality});
    ...
    controls.enabled=!worldTouch.state.enabled;controls.update(visualDt);if(worldTouch.state.active)cameraGesture();choreographCamera(visualDt,time);optics.update({...performanceLook.optics,time:phase,motion,quality:autoLow?'low':quality});composer.render();
    ...
    frames++;Object.assign(diagnostics,{frames,energy,peak,bass,mid,treble,model,bloomAvailable:bloom.enabled,collection:collection.diagnostics,spinAngle:rotor.rotation.z,spinVelocity,rpm,spin,autoOrbit,cameraAzimuth:controls.getAzimuthalAngle(),world:currentWorld,reactivity,look:{...look},labLook:{...labLook},signals:{bass:signalBass,mid:signalMid,treble:signalTreble,energy,pulse:musical.pulse,centroid:musical.centroid},performance:{...performanceLook,cameraPhase}});
