...
Yeni, bağımsız 3D player: **http://127.0.0.1:4176/player-three.html**. Bu klasördeki **START-PLAYER.cmd** ile açılır. Beğenilen Beyond sürümü `website_html_templates_lab` içinde **4175** adresinde, ilk player ise `website_html_templates` içinde **4173** adresinde korunur. Living Worlds kendi tarayıcı kayıtlarını kullanır.

- **Tidal:** üç boyutlu kıyılar, yansımalar, köpük ve su altındaki ışıklı akıntılar. Müziğin düşük frekansları yüzeyi hareket ettirir; dokunuşlar ayrı halkalar ve renkli izler bırakır.
- **Monolith:** anıtsal mekânın merkezinde açılan kanatlar, hareketli pistonlar, katmanlı taşıyıcı halkalar ve parçalı kristal. Merkez hem müziğe hem el hareketine tepki verir.
- **Aether:** dallanan ışık kanalları, aralarında derinlik oluşturan bulutsu katmanları ve dokunarak bükülebilen akımlar. Uyumlu cihazlarda parçacık konumları GPU üzerinde hesaplanır.
- **Shape with touch:** aç, ardından dünyanın üzerinde basılı tutup sürükle. Müzik duraklatılmışken de çalışır. Kapatınca sürükleme yeniden kameraya geçer; Motion kapalıyken dokunma da durur.
...
Windows’ta **START.cmd** dosyasını aç. Yerel önizleme: **http://127.0.0.1:4176**. Bu bilgisayardaki Node.js veya Codex ile gelen Node.js otomatik bulunur.
