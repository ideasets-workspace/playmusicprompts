# Beyond Living Worlds — Three.js player

Current edition: **Living Worlds**, the independent successor at **http://127.0.0.1:4176/player-three.html**. Use **START-PLAYER.cmd** to reopen it. The accepted Beyond edition at 4175 and original player at 4173 are preserved separately. This edition adds sculpted coasts and luminous water interaction in Tidal, an articulated mechanical heart in Monolith, and branching light with a continuous nebular medium in Aether. Enable **Shape with touch**, then hold and drag across a world; disable it to return dragging to the camera. Motion off freezes the animated scene and disables touch. See this folder's README for the current guide and `docs/design/2026-09-12-beyond-living/brief.md` in the project root for scope.

The following sections document the inherited player and its earlier delivery stages. Their historical four-scene counts and verification references do not describe the complete seven-scene Living Worlds edition.
...
Open **http://127.0.0.1:4176/player-three.html** while the local preview is running. Otherwise open **START-PLAYER.cmd**. Node.js is required to serve the folder. The separate player preserves the five approved website pages and the original player layout.
...
Playable `PMP_CONFIG.catalog` entries and browser-local `pmp.beyond.creations` remain integration points. Audio shares IndexedDB `pmp-beyond-local-audio`, version 1, store `tracks`, with the HTML suite. Favorites share `pmp.beyond.saved`; room preferences use `pmp.beyond.room.preferences.v2`, session ownership/order/position use `pmp.beyond.room.session.v2`. The legacy `pmp.beyond.room` is migration input only. Sound, presets, timer and bookmarks use dedicated `pmp.beyond.room.*` keys. Navigation from another player starts a separate playback session. Reload restores selection without autoplay.
