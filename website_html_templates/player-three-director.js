...
const sceneIds = new Set(['record', 'aurora', 'orbital', 'liquid']);
...
export const DIRECTOR_STORAGE_KEY = 'pmp.room.director.v1';
  ...
  if (Array.isArray(suppliedCues)) {
    ...
    for (const [index, cue] of boundedList(suppliedCues, 24).entries()) {
      ...
      byTime.set(normalizedTime, {id, time: normalizedTime, label: stringLabel(valueOf(cue, 'label'), `Cue ${index + 1}`),
        scene, settings: sanitizeVisualSettings(settings), duration: bounded(duration, 0, 30, 0)});
    }
  }
...
function validPatch(patch) {
  ...
  if (has('cues') && !get('cues').every(cue => fields(cue, ['id', 'time', 'label', 'scene', 'settings', 'duration'])
    && safeId(valueOf(cue, 'id')) && finite(valueOf(cue, 'time')) && finite(valueOf(cue, 'duration'))
    && typeof valueOf(cue, 'label') === 'string' && sceneIds.has(valueOf(cue, 'scene')) && validSettings(valueOf(cue, 'settings')))) return false;
  return true;
}
...
export function createDirectorEngine() {
  ...
  return {
    ...
    update(frame = {}) {
      ...
      let settings = {...base}, outputScene = inputScene, activeCueId = null, status = 'Manual look';
      if (config.mode === 'morph') {
        ...
        if (!config.scoreTrackId) status = 'Score needs a track';
          ...
          if (index < 0) status = 'Before the first cue';
            ...
            outputScene = cue.scene; activeCueId = cue.id;
      ...
      previous = {settings: sanitizeVisualSettings(settings), scene: outputScene, xy: {...xy}, signals: {...levels},
        activeCueId, status, mode: config.mode, optics: {...config.optics}, camera: {...config.camera}};
